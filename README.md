...
`scripts/docker-build.sh` 会先构建 `xxl-job-admin-ui` 的生产产物，再用 JDK 17 打包后端，最后生成 `wujiawei0926/xxl-job-boost-admin:local`、`wujiawei0926/xxl-job-boost-all-in-one:local` 和样例执行器镜像。all-in-one 只用于全新数据库，不会自动迁移官方旧库。
