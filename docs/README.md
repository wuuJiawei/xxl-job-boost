# XXL-JOB Boost 文档

仓库文档统一收录在 `docs/`，不再同时维护 `doc/` 和 `docs/` 两套目录。

## 使用与迁移

- [从官方 XXL-JOB 迁移到 XXL-JOB Boost](./migration-from-xxl-job.md)
- [数据库脚本说明](./db/README.md)
- [本地运行与排障](./local-dev-runbook.md)
- [生产部署方案](./production-deployment.md)
- [Boost 已落地能力](./boost-features.md)
- [1.0.0 测试计划](./test-plan-1.0.0.md)

## 设计与发布

- [功能路线图](./feature-roadmap.md)
- [管理后台迁移计划](./admin-ui-migration-plan.md)
- [源码增强策略](./upstream-extension-strategy.md)
- [1.0.0 发布说明](./release-notes-1.0.0.md)
- [早期版本说明](./release-notes-2026-06-10.md)

## 需求记录

- [需求文档索引](./requirements/README.md)

## 上游资料镜像

- [官方中文文档](./upstream/XXL-JOB官方文档.md)
- [Official English Documentation](./upstream/XXL-JOB-English-Documentation.md)
- `upstream/images/`：上游文档历史图片资源
- `upstream/XXL-JOB架构图.key`：上游架构图源文件
