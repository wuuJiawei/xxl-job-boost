...
- 旧控制台关闭、`admin-next` 唯一页面入口和首次登录强制改密。
...
| 编号 | 用例 | 前置条件 | 步骤 | 期望结果 | 优先级 |
| --- | --- | --- | --- | --- | --- |
| UI-001 | 旧控制台关闭 | admin 已启动 | 打开 `/xxl-job-admin/` | 返回 404，不渲染旧 Freemarker 控制台 | P0 |
| UI-002 | 首次登录强制改密 | admin 已启动 | 打开 `/xxl-job-admin/admin-next/`，用 `admin/123456` 登录 | 进入强制改密页，弱密码被拒绝，强密码成功后重新登录 | P0 |
| UI-003 | 新控制台入口 | 已登录 | 访问 `/xxl-job-admin/admin-next/` | 页面可访问，API 正常返回 | P0 |
