...
当前正在运行官方 Docker `3.0.0`、希望执行器暂不升级且保持运行时，直接按[官方 XXL-JOB 3.0.0 迁移到 Boost 0.9.4 SOP](./sop-migrate-official-3.0.0-to-boost-0.9.4.md)执行。

> 版本基线：截至 2026-07-19，官方最新 release 为 `3.4.2`。本仓库提供官方 `3.4.2`、`3.0.0` 和官方 `2.4.x / 2.5.x` 三条迁移 SQL。官方 `2.3.x` 及更早版本不在直迁范围内，必须先升级到 `2.4.2` 或 `2.5.0`。
...
该脚本会先完成官方 3.4.2 结构对齐：

- `xxl_job_group.title` 扩到 `varchar(64)`。
- `xxl_job_registry.id` 扩到 `bigint`，保留唯一注册键。
- 任务和日志的 `executor_param` 调整为 `text`。
- `xxl_job_user.password` 扩到 `varchar(100)`并增加 `token`。
- 日志查询索引对齐为 `I_jobgroup` 和 `I_jobid`。

随后执行与 3.4.2 路径相同的 Boost 增量。最后一行应包含：

```text
XXL-JOB official 3.0.0 -> XXL-JOB Boost database migration completed
```

5. 处理密码。官方 3.0.0 仍使用 MD5，脚本输出 `users_requiring_password_reset`。至少先为管理员设置一个新的强密码：
...
```bash
java -jar xxl-job-admin/target/xxl-job-admin-0.9.4.jar \
  --server.port=8080 \
  --server.servlet.context-path=/xxl-job-admin \
...
- 不确认来源版本，同时执行多份迁移 SQL。
- 3.0.0 或 2.x 迁移后没有重置 MD5 密码，导致所有用户无法登录。
