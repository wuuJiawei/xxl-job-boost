export const GLUE_INIT_REMARK = 'GLUE code initialized by Admin Next';

const glueTemplates: Record<string, string> = {
...
const glueEditorModes: Record<string, string> = {
  GLUE_GROOVY: 'text/x-java',
  GLUE_SHELL: 'text/x-sh',
  GLUE_PYTHON: 'text/x-python',
  GLUE_PYTHON2: 'text/x-python',
  GLUE_PHP: 'application/x-httpd-php',
  GLUE_NODEJS: 'text/javascript',
  GLUE_POWERSHELL: 'application/x-powershell'
};

export function isGlueType(glueType: string) {
  return glueType.startsWith('GLUE_');
}

export function getGlueTemplate(glueType: string) {
  return glueTemplates[glueType] || '';
}

export function getGlueEditorMode(glueType: string) {
  return glueEditorModes[glueType] || 'text/plain';
}
